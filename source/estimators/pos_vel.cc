...
namespace sdrobot::est
{
  ...
  bool PosVel::Setup()
  {
    ...
    A_.setZero();
    A_.block<3, 3>(0, 0) = Matrix3::Identity();
    A_.block<3, 3>(0, 3) = dt * Matrix3::Identity();
    A_.block<3, 3>(3, 3) = Matrix3::Identity();
    A_.block<12, 12>(6, 6) = Matrix12::Identity();
    //输入矩阵
    B_.setZero();
    B_.block<3, 3>(3, 0) = dt * Matrix3::Identity();
    //观测矩阵
    ...
    C_.block<3, 6>(6, 0) = C1;
    C_.block<3, 6>(9, 0) = C1;
    C_.block<12, 12>(0, 6) = -1. * Matrix12::Identity();
    ...
    //初始状态估计噪声
    Q0_.setIdentity();
    Q0_.block<3, 3>(0, 0) = (dt / 20.) * Matrix3::Identity();
    Q0_.block<3, 3>(3, 3) =
        (dt * 9.8 / 20.) * Matrix3::Identity();

    Q0_.block<12, 12>(6, 6) = dt * Matrix12::Identity();
  ...
  bool PosVel::Run(StateData &ret, const robot::leg::Datas &datas, const robot::QuadrupedPtr &quad)
  {
    ...
    for (int i = 0; i < robot::ModelAttrs::num_leg; i++)
    {
      ...
      Vector3 dp_rel = datas[i].v; //足端速度在机身坐标系
    ...
    Matrix18 At = A_.transpose();

    Matrix18 Pm = A_ * P_ * At + Q; //不确定性预测方程

    //卡尔曼增益准备
    Eigen::Matrix<double, 18, 28> Ct = C_.transpose();

    Vector28 yModel = C_ * xhat_; //预测的观测值

    Vector28 ey = y - yModel; //误差 卡尔曼增益计算准备

    Matrix28 S = C_ * Pm * Ct + R; //卡尔曼增益计算准备
    ...
    P_ = (Matrix18::Identity() - Pm * Ct * S_C) * Pm; //最佳估计不确定性协方差??

    Matrix18 Pt = P_.transpose(); //??
    P_ = (P_ + Pt) / 2.;                        //??
