...
bool FloatBaseModelImpl::ComputeContactJacobians() {
  ...
  for (int k = 0; k < n_ground_contact_; k++) {
    ...
    if (!compute_contact_info_[k]) continue;
...
bool FloatBaseModelImpl::ForwardKinematics() {
  ...
  // TODO(Michael Ding) : we end up inverting the same Xa a few times (like for
  // the 8 points on the body). this isn't super efficient.
