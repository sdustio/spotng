...
TaskLinkPos::TaskLinkPos(model::FloatBaseModel::ConstSharedPtr const &model, SdVector3f const &kp, SdVector3f const &kd,
                         int contact_idx, bool virtual_depend)
    : Task(model, kp, kd), contact_idx_(contact_idx), virtual_depend_(virtual_depend) {
  // kp_foot, kd_foot
}

bool TaskLinkPos::_UpdateCommand(SdVector3f const &pos_des, SdVector3f const &vel_des, SdVector3f const &acc_des) {
  auto const &link_pos = robot_sys_->GetGroundContactPos()[contact_idx_];
...
bool TaskLinkPos::_UpdateTaskJacobian() {
  Jt_ = robot_sys_->GetContactJacobians()[contact_idx_];
  if (!virtual_depend_) {
...
bool TaskLinkPos::_UpdateTaskJDotQdot() {
  JtDotQdot_ = robot_sys_->GetContactJacobiansdqd()[contact_idx_];
  return true;
}
