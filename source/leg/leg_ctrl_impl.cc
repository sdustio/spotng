...
namespace sdrobot::leg
{
  ...
  bool LegCtrlImpl::SendCmdsToActuatorInterface()
  {
    ...
    for (int leg = 0; leg < params::model::num_leg; leg++)
    {
      ...
      foot_force += ToConstEigenTp(cmds_[leg].kp_cartesian) * (ToConstEigenTp(cmds_[leg].p_des) - ToConstEigenTp(datas_[leg].p));

      foot_force += ToConstEigenTp(cmds_[leg].kd_cartesian) * (ToConstEigenTp(cmds_[leg].v_des) - ToConstEigenTp(datas_[leg].v));
