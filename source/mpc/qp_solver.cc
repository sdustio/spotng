...
bool near_one(double a) { return near_zero(a - 1); }
}  // namespace
...
bool QPSolver::SolveQP(fpt_t const x_drag, SdVector3f const &pos, SdVector3f const &lvel, SdVector4f const &ori,
  ...
  for (int rs = 0; rs < 3; rs++)
    for (int c = 0; c < 4; c++) r_feet(rs, c) = rel_foot_p[rs * 4 + c];
  Matrix3 I_body, R_yaw;
  auto yc = cos(yaw);
  auto ys = sin(yaw);
  R_yaw << yc, -ys, 0, ys, yc, 0, 0, 0, 1;
  I_body.diagonal() << .07, 0.26, 0.242;  // TODO(Micheal Ding) read from options
  ...
  for (int i = 0; i < 12; i++) full_weight(i) = weights[i];
  full_weight(12) = 0.;
  ...
  for (int i = 0; i < num_constraints; i++) {
    if (!(near_zero(qlb_[i]) && near_zero(qub_[i]))) continue;
    auto c_row = qA.row(i);
