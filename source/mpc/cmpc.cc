...
bool CMpc::RunOnce(wbc::InData &wbcdata, leg::LegCtrl::SharedPtr const &legctrl,
  ...
  if (cmd_gait == drive::Gait::FlyingTrot) _body_height = 0.31;
...
bool CMpc::UpdateMPCIfNeeded(std::array<SdVector3f, 4> &out, std::vector<int> const &mpcTable,
  ...
  if (xStart - pos[0] > max_pos_error) xStart = pos[0] + max_pos_error;
  if (pos[0] - xStart > max_pos_error) xStart = pos[0] - max_pos_error;

  if (yStart - pos[1] > max_pos_error) yStart = pos[1] + max_pos_error;
  if (pos[1] - yStart > max_pos_error) yStart = pos[1] - max_pos_error;
  ...
  for (int i = 0; i < opts::horizon_len; i++) {
    for (int j = 0; j < 12; j++) traj_all_[12 * i + j] = trajInitial[j];

    if (i == 0) {
      // TODO(Michael Ding) consider not doing this
...
bool CMpc::SolveMPC(std::array<SdVector3f, 4> &out, std::vector<int> const &mpcTable,
  ...
  for (int i = 0; i < 12; i++) r[i] = p_foot_[i % 4][i / 4] - pos[i / 4];
  ...
  for (int leg = 0; leg < 4; leg++) {
    SdVector3f f;
    for (int axis = 0; axis < 3; axis++) f[axis] = solu[leg * 3 + axis];
