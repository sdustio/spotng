...
namespace sd::dynamics
{
  ...
  BSpline::BSpline()
      : num_knots_(DEGREE + NUM_MIDDLE + 2 + CONST_LEVEL_INI + CONST_LEVEL_FIN +
                   1),
        num_cps_(NUM_MIDDLE + 2 + CONST_LEVEL_INI + CONST_LEVEL_FIN)
  {
    for (int i(0); i < num_knots_; ++i)
      knots_[i] = 0.;
    for (int i(0); i < num_cps_; ++i)
    {
      for (int j(0); j < DIM; ++j)
        cpoints_[i][j] = 0.;
    }
    assert(num_knots_ >= 2 * (DEGREE + 1));
  }

  bool BSpline::SetParam(
      const std::array<double, 3 * DIM> &init, const std::array<double, 3 * DIM> &fin,
      const std::array<std::array<double, DIM>, 1> &middle_pt, double fin_time)
  {
    CalcKnot(fin_time);
  ...
  bool BSpline::GetCurvePoint(double u, std::array<double, DIM> &ret)
  {
    int _span;
    ...
    std::array<double, DEGREE + 1> _N;
    BasisFuns(_N, _span, u);

    std::array<double, DIM> _C;
  ...
  bool BSpline::GetCurveDerPoint(double u, int d, std::array<double, DIM> &ret)
  {
    if (d > DEGREE)
      return 0.0;
    ...
    std::vector<std::array<double, DIM>> _CK(d + 1);
  ...
  void BSpline::CalcKnot(double Tf)
  {
    int _i(0);
    int _j(0);
    int _NumMidKnot(num_knots_ - 2 * DEGREE - 2);
  ...
  bool BSpline::CurveDerivsAlg1V(std::vector<std::array<double, DIM>> &CK, double u, int d)
  {
    assert(d <= DEGREE);
    ...
    std::vector<std::array<double, DEGREE + 1>> _nders(d + 1);
  ...
  bool BSpline::BasisFunsDers(
      std::vector<std::array<double, CONST_LEVEL_INI + 2>> &ders,
      double u, int n)
  {
      ...
      int span, double u, int n)
  {
    ...
    std::array<std::array<double, DEGREE + 1>, DEGREE + 1> _ndu;

    // to store (in an alternating fashion) the two most recently computed
    // rows a(k,j) and a(k-1,j)
    std::array<std::array<double, DEGREE + 1>, 2> _a;
  ...
  void BSpline::BasisFuns(std::array<double, DEGREE + 1> &N, double u)
  {
    // Original
    /*int _span = FindSpan(u);
      BasisFuns(N, _span, u);*/
  ...
  void BSpline::BasisFuns(std::array<double, DEGREE + 1> &N, int span, double u)
  {
    int _j, _r;
    double _left = 0.0;
    double _right = 0.0;
      ...
      double Tf)
  {
    ...
    // Initial Constraints
    std::vector<std::array<double, DEGREE + 1>> d_mat(CONST_LEVEL_INI + 1);

    BasisFunsDers(d_mat, 0., CONST_LEVEL_INI);

    std::array<double, DIM> ini_const;
    // Vel, Acc, ...
    ...
    // Final Constraints
    std::vector<std::array<double, DEGREE + 1>> c_mat(CONST_LEVEL_FIN + 1);

    BasisFunsDers(c_mat, Tf, CONST_LEVEL_FIN);

    // Vel, Acc, ...
    int idx(1);
    for (int j(num_cps_ - 2); j > num_cps_ - 2 - CONST_LEVEL_FIN; --j)
    {
      for (int k(0); k < DIM; ++k)
      {
        ...
        {
          ini_const[k] -=
              c_mat[idx][CONST_LEVEL_FIN + 2 - h] * cpoints_[num_cps_ - h][k];
        }
  ...
  void BSpline::CalcCPoints(const std::array<std::array<double, DIM>, NUM_MIDDLE> &middle_pt)
  {
