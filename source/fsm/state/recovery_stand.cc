...
constexpr inline std::array<SdVector3f, 4> const rolling_jpos = {
    SdVector3f{1.5, -1.6, 2.77}, SdVector3f{1.3, -3.1, 2.77},
    SdVector3f{1.5, -1.6, 2.77}, SdVector3f{1.3, -3.1, 2.77}};
...
bool StateRecoveryStand::StandUp(const int curr_iter) {
  ...
  if (curr_iter <= floor(opts::standup_ramp_iter * 0.7)) {
    for (int leg = 0; leg < consts::model::kNumLeg; ++leg) {
      SetJPosInterPts(curr_iter, opts::standup_ramp_iter, leg,
                      initial_jpos_[leg], opts::stand_jpos[leg]);
    }
...
bool StateRecoveryStand::FoldLegs(const int curr_iter) {
  for (int i = 0; i < consts::model::kNumLeg; ++i) {
    SetJPosInterPts(curr_iter, opts::fold_ramp_iter, i, initial_jpos_[i],
                    opts::fold_jpos[i]);
  }
...
bool StateRecoveryStand::RollOver(const int curr_iter) {
  for (int i = 0; i < consts::model::kNumLeg; ++i) {
    SetJPosInterPts(curr_iter, opts::rollover_ramp_iter, i, initial_jpos_[i],
                    opts::rolling_jpos[i]);
  }
...
bool StateRecoveryStand::SetJPosInterPts(int const curr_iter,
                                         int const max_iter, int const leg,
                                         SdVector3f const &ini,
...
bool StateRecoveryStand::JointPDControl(int const leg, SdVector3f const &qDes,
