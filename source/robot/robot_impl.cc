...
RobotImpl::RobotImpl(Options const &opts,
  ...
  fsm_ = std::make_shared<fsm::FiniteStateMachineImpl>(opts, legctrl_, mquad_,
                                                       drivectrl_, estctrl_);
}

bool RobotImpl::UpdateImu(sensor::ImuData const &imu) {
  std::dynamic_pointer_cast<estimate::Orientation>(
      estctrl_->GetEstimator("ori"))
      ->UpdateImu(imu);
  return true;
}
...
bool Robot::Build(Ptr &ret, Options const &opts,
                  interface::ActuatorInterface::SharedPtr const &act_itf) {
  ret = std::make_unique<RobotImpl>(opts, act_itf);
  return true;
}

bool Robot::Build(SharedPtr &ret, Options const &opts,
                  interface::ActuatorInterface::SharedPtr const &act_itf) {
