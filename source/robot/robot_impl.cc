...
bool RobotCtrlImpl::ParseOptions(Options::SharedPtr const &opts) {
  ...
  if (logt == logging::Target::Console) {
    logger = spdlog::stdout_color_mt("sdengine");
  } else if (logt == logging::Target::File) {
    logger = spdlog::basic_logger_mt("sdengine", opts->log_filename);
  } else if (logt == logging::Target::RotateFile) {
    logger = spdlog::rotating_logger_mt("sdengine", opts->log_filename, opts->log_max_file_size, opts->log_max_files);
  }
